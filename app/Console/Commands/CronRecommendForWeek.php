...
class CronRecommendForWeek extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cron:recommend:for:week';
    ...
    public function handle()
    {
        ...
        // Write your database logic we bellow:
         ...
         $data = CronRecommendPoemWeekDay::get();
         $message = [
             ...
             if($val['type'] == 1) {
                 ...
                 if ($currentUser['recommend_poem_email'] == 1) {
                     ...
                     Mail::send('backend.emails.sendEmailPerDay',['name'=>$user_name,'email'=>$email,'poemDetail'=>$poemDetail,'poemtitle'=>$poemtitle,'poemauthor'=>$poemauthor,'poempublication'=>$poempublication,'user_id'=>$val['userid'],'token'=> $tokengen, 'other' => $other],function($message) use($email,$project_name){
                         $message->to($email,$project_name)->subject('Your weekly recommended poem');
                 ...
                 CronRecommendPoemWeekDay::where('id',$val->id)->update(['status' => 2]);
             } else if ($val['type'] == 2) {
                 if ($val['mobile_token']) {
                     // Get poemText from Python script
